...
class GAN(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        return [{'optimizer': opt_g, 'frequency': 5}, {'optimizer': opt_d, 'frequency': 1}]
    ...
    def _gradient_penalty(self, real: Batch, fake: Batch, center: float = 1.):
        ...
        interp = Data(pos=(alpha * real.pos + (1 - alpha) * fake.pos).requires_grad_(), batch=batch)
    ...
    def train_dataloader(self):
        ds = ModelNet('/mnt/tank/scratch/dleonov/data/modelnet',
                      name='10',
                      transform=T.Compose([T.NormalizeScale(), T.RandomRotate(20, axis=2), T.SamplePoints(1024)]))
        label = ds.raw_file_names.index(self.config.label)
        ds = [d for d in ds if d.y == label]
